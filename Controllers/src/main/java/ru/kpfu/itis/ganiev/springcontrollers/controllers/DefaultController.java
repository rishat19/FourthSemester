package ru.kpfu.itis.ganiev.springcontrollers.controllers;
...
public class DefaultController {
    ...
    public String requestMappingBunch(@RequestParam(
            value = "secret_param"
    ) String param, ModelMap map) {
        if (param.equals("secret")) {
            map.put("viewVariable", "Page with secret param for some browsers: " + param);
        }
        return "tests";
    }
    ...
    public String requestHeader(@RequestHeader() Map<String, String> headers, ModelMap map) {
        List<Header> headersResult = new ArrayList<>();
        headers.forEach((key, value) -> headersResult.add(new Header(key, value)));
